...
export const RAC_LABEL={1:'High',2:'Serious',3:'Medium',4:'Low',5:'Low'};
export const RAC_BAND={1:'high',2:'serious',3:'medium',4:'low',5:'low'};
...
export function renderDeaRiskMatrix(systemSeverity='',systemProbability=''){
 ...
 const rows=Object.entries(RAC_SEVERITY).map(([s,v])=>`<tr><th scope="row"><button type="button" class="rac-choice severity-choice${s===systemSeverity?' system-choice':''}" data-severity="${s}"><strong>${s} — ${v.name}</strong><span>${v.definition}</span></button></th>${probs.map(([p])=>{const code=racCode(s,p),band=RAC_BAND[code];return`<td class="matrix-intersection risk-band-${band}" data-cell="${s}${p}"><span>${RAC_LABEL[code]}</span></td>`}).join('')}</tr>`).join('');
 return `<div class="rac-workspace"><div class="rac-intro"><div><span class="rac-kicker">DEA™ RISK CONTROL</span><h3>Risk Assessment Code (RAC)</h3><p>Choose one Severity and one Probability. DEA calculates the RAC automatically. Definitions are built into the matrix so the assessment can be performed without a separate legend.</p></div><div class="system-rac"><span>SYSTEM RAC</span><strong>${systemSeverity&&systemProbability?`RAC ${racCode(systemSeverity,systemProbability)}`:'Not assessed'}</strong><small>${systemSeverity&&systemProbability?`${systemSeverity}/${systemProbability} — ${RAC_LABEL[racCode(systemSeverity,systemProbability)]}`:'Awaiting system assessment'}</small></div></div><div class="dea-matrix-wrap"><table class="dea-matrix"><caption>Dominion1st™ Risk Assessment Matrix</caption><thead><tr><th>SEVERITY</th>${probs.map(([p,v])=>`<th><button type="button" class="rac-choice probability-choice${p===systemProbability?' system-choice':''}" data-probability="${p}"><strong>${p} — ${v.name}</strong><span>${v.definition}</span></button></th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div><div class="risk-band-key"><span class="risk-band-high">HIGH</span><span class="risk-band-serious">SERIOUS</span><span class="risk-band-medium">MEDIUM</span><span class="risk-band-low">LOW</span></div><div class="rac-result" id="racUserResult"><span>USER ASSESSMENT</span><strong>Select Severity + Probability</strong><p>RAC = Severity × Probability matrix intersection.</p></div><p class="dea-matrix-note">System RAC remains authoritative for DEA™. A user assessment may be saved or challenged, but it does not silently change the System RAC.</p></div>`;
}
