...
async function s1LoadCoachCalendar(){
 ...
 try{const ids=roster.map(x=>x.id),range=s1WeekRange(window.s1CalOffset||0),f=range.from,t=range.to;const label=document.getElementById('coachCalendarLabel');if(label)label.textContent=range.label;const [rows,events]=await Promise.all([s1('spartans1st_training_assignments?select=athlete_id,training_date,title,status&athlete_id=in.('+ids.join(',')+')&training_date=gte.'+f+'&training_date=lte.'+t+'&order=training_date'),s1CalendarEvents(ids,f,t)]);const names=Object.fromEntries(roster.map(x=>[x.id,x.display_name]));const items=[...(rows||[]).map(x=>({date:x.training_date,athlete_id:x.athlete_id,title:x.title,type:x.status||'assigned'})),...(events||[]).map(x=>({date:x.event_date,athlete_id:x.athlete_id,title:x.title,type:x.event_type}))].sort((a,b)=>a.date.localeCompare(b.date));box.innerHTML=items.length?items.map(x=>'<div class="row"><span><b>'+esc(x.date)+'</b> · '+esc(names[x.athlete_id]||'Athlete')+' · '+esc(x.title)+'</span><span class="pill">'+esc(x.type).replace('_',' ').toUpperCase()+'</span></div>').join(''):'<p>No authorized activities scheduled this week.</p>'}catch(e){box.innerHTML='<p>Team schedule could not be loaded.</p>'}
}
...
async function s1LoadParentCalendar(){const box=document.getElementById('parentCalendarItems');if(!box||!window.s1ParentAthleteId)return;const range=s1WeekRange(window.s1CalOffset||0),label=document.getElementById('parentCalendarLabel');if(label)label.textContent=range.label;try{const events=await s1CalendarEvents([window.s1ParentAthleteId],range.from,range.to);const rows=(events||[]).filter(x=>x.visibility==='athlete_coach_parent');box.innerHTML=rows.length?rows.map(x=>'<div class="row"><span><b>'+esc(x.event_date)+'</b> · '+esc(x.title)+'</span><b>'+esc(x.event_type).replace('_',' ').toUpperCase()+'</b></div>').join(''):'<p class="muted">No parent-visible events this week.</p>'}catch(e){box.innerHTML='<p class="muted">Calendar could not be loaded.</p>'}}
