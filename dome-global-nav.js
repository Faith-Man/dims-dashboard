(() => {
  ...
  const destinations = [
    ...
    ['OIKONOMOS™', 'oikonomos/'],
    ['EKKLĒSIA™', 'ekklesia/'],
    ['KUBERNĒSIS / Mission Control', 'dashboard-v3-current.html'],
  ...
  style.textContent = `
    ...
    .dome-global-drawer{position:absolute;left:0;top:0;width:min(360px,88vw);height:100%;background:#fff;transform:translateX(-102%);transition:transform .2s ease;box-shadow:18px 0 48px rgba(0,0,0,.28);display:flex;flex-direction:column}
    ...
    .dome-global-list{padding:12px;overflow:auto;display:grid;gap:7px}
    ...
    @media(min-width:1000px){.dome-global-drawer{width:340px}.dome-global-bar{padding-left:20px;padding-right:20px}}
  `;
  ...
  backdrop.innerHTML = `<aside class="dome-global-drawer" role="dialog" aria-modal="true" aria-label="DOME navigation"><div class="dome-global-head"><div><strong>DOME™ Navigation</strong><small>Go anywhere in the current system</small></div><button class="dome-global-close" type="button" aria-label="Close navigation">×</button></div><nav class="dome-global-list">${destinations.map(([label,path]) => `<a class="dome-global-link${isActive(to(path))?' active':''}" href="${to(path)}">${label}</a>`).join('')}</nav></aside>`;
