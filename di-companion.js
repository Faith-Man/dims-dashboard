(() => {
  ...
  if (page === 'projects-tasks.html') {
    ...
    const openStagePopover = target => {
      ...
      pop.innerHTML = `<button class="close" type="button" aria-label="Close explanation">×</button><h3>RAD™ — Risk Assessment Dome</h3><div class="code">${code}</div><p><strong>RAC — Risk Assessment Code</strong> assesses risk from Severity × Probability. RAD supports physical and non-physical governed risk.</p><p><strong>DEA™ — DIMS Execution Algorithm</strong> determines execution order after RAC and required gates. When items share the same RAC, DEA uses objective recorded system facts such as verified blocking dependencies, recorded deadlines, waiting age, readiness, and continuity. The former EPI is retired as a separate active layer.</p><p class="metric-note"><strong>Execution Performance Metrics</strong> — HEI, AEL, RWL, VOY, Autonomy, and Session Window — measure how work was actually executed. They are observed during/after execution and do not determine RAC or replace DEA priority ordering.</p><p class="boundary"><strong>APN — Abatement Priority Number: ACTUAL PHYSICAL HAZARDS ONLY.</strong> Use it for physical occupational-safety, fire, or occupational-health hazards where personnel exposure and abatement cost are meaningful. Do not use APN for ordinary software, project, governance, deployment, administrative, or workflow risk.</p><div class="quick-calc"><strong>Quick RAC Calculator</strong><div class="quick-row"><span>Severity:</span><button data-qsev="I">I</button><button data-qsev="II">II</button><button data-qsev="III">III</button><button data-qsev="IV">IV</button></div><div class="quick-row"><span>Probability:</span><button data-qprob="A">A</button><button data-qprob="B">B</button><button data-qprob="C">C</button><button data-qprob="D">D</button></div><div class="quick-result" data-quick-result></div></div><a class="guide" href="rac-epi-apn-guide.html">Open Full RAD Guide</a>`;
  ...
  button.className='di-launch'; button.type='button'; button.textContent='✦ DI²'; button.setAttribute('aria-haspopup','dialog'); button.setAttribute('aria-label','Open DI²');
  ...
  function render(){const rows=messages();log.innerHTML=rows.length?rows.map(m=>`<div class="di-message ${escapeHtml(m.role)}">${escapeHtml(m.text)}${m.citations?.length?`<ul class="di-citations">${m.citations.map(c=>`<li><a href="${escapeHtml(c.url)}">${escapeHtml(c.number)} — ${escapeHtml(c.title)}</a></li>`).join('')}</ul>`:''}</div>`).join(''):'<div class="di-message assistant">Ask, analyze, explain, or direct across DOME or this module. DI² will identify what is live, calculated, recommended, or unavailable.</div>';log.scrollTop=log.scrollHeight}
  ...
  panel.querySelector('form').addEventListener('submit',async e=>{
    ...
    }catch(error){history.push({role:'error',text:`DI² is unavailable: ${error.message}`})}finally{save(history);render();send.disabled=false;send.textContent='DI²'}
  });
})();