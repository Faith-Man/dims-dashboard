...
const DOME_DEPLOY_MARKER = '2026-08-23T03:52-05:00-tetelestai-browser-recovery';
...
async function injectTetelestaiRecovery(response, originalPath) {
  ...
  const marker = '<script type="module" src="tetelestai-recovery-loader.js?v=20260823-0352"></script>';
  const body = html.includes('tetelestai-recovery-loader.js') ? html : html.replace('</body>', `${marker}</body>`);
  const headers = new Headers(response.headers);
  headers.delete('content-length');
  headers.set('X-DOME-TETELESTAI-Recovery', 'armed');
